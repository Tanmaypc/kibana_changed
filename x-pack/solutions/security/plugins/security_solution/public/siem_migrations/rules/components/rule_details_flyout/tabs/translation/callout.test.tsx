...
describe('TranslationCallOut', () => {
  ...
  it('renders correctly for "mapped" mode', () => {
    ...
    expect(getByTestId('ruleMigrationCallOut-mapped')).toHaveTextContent(
      'This rule was mapped to a Cyberstanc-authored rule. Click Install & enable rule to complete migration. You can fine-tune it later.'
    );
  });
  ...
  it('renders correctly for "partial" mode', () => {
    ...
    expect(getByTestId('ruleMigrationCallOut-partial')).toHaveTextContent(
      'Part of the query could not be translated.To save this rule, finish writing the query. If you need help, contact Cyberstanc support.'
    );
  });
